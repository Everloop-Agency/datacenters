(() => {
  ...
  // Data-center climate risk explorer.
  // - Google Satellite 2D with Esri fallback
  // - fast 20 km flood and wildfire queries
  // - WeatherTradeNet scenario analysis
  // - optional Google Photorealistic 3D
  // - distance scale, measurement tool, and PLAY tour
  ...
  // The nationwide flood layer below is derived from FEMA NFHL and is the
  // primary national source. The separate direct FEMA endpoint and heavy
  // historical archives were removed because they were slow/redundant and
  // frequently failed in the browser.
  ...
  function ensureMapLoadingUi() {
    if (mapLoadingEl) return;

    const style = document.createElement("style");
    style.textContent = `
      #mapLoadingOverlay[hidden]{display:none!important}
      #mapLoadingOverlay{
        position:absolute;z-index:1450;top:20px;left:50%;transform:translateX(-50%);
        display:flex;align-items:center;gap:10px;padding:10px 15px;border-radius:12px;
        background:rgba(247,249,255,.97);color:#424656;font:700 13px/1.2 Inter,Arial,sans-serif;
        box-shadow:0 8px 24px rgba(0,0,0,.22);pointer-events:none;white-space:nowrap
      }
      #mapLoadingOverlay .map-loading-spinner{
        width:20px;height:20px;box-sizing:border-box;border:3px solid rgba(159,116,151,.24);
        border-top-color:rgba(159,116,151,.98);border-radius:50%;animation:dc-map-spin .7s linear infinite
      }
      @keyframes dc-map-spin{to{transform:rotate(360deg)}}
    `;
  ...
  async function loadCoreFloodPolygons(location, generation) {
    const bbox = bboxAround(location, HAZARD_RADIUS_KM);
    const tasks = [];
    const labels = [];
    ...
    // Preserve the proven local services where available.
    if (inAbilene) {
      tasks.push(fetchGeoJson(
        arcGisGeoJsonUrl(`${ABILENE_FLOOD_SERVICE}/1/query`, bbox),
        "Abilene flood layer 1"
      ));
      labels.push("City of Abilene flood zones");

      tasks.push(fetchGeoJson(
        arcGisGeoJsonUrl(`${ABILENE_FLOOD_SERVICE}/0/query`, bbox),
        "Abilene flood layer 0"
      ));
      labels.push("City of Abilene flood zones");
    }
    ...
    if (inElPaso) {
      tasks.push(fetchGeoJson(
        arcGisGeoJsonUrl(EL_PASO_FLOOD_SERVICE, bbox, { outFields: "*" }),
        "El Paso flood"
      ));
      labels.push("City of El Paso flood zones");
    }

    // Primary national FEMA-derived flood layer. Restrict to 20 km and at most
    // two pages so PLAY is not blocked by large downloads.
    tasks.push(fetchGeoJsonPaged(
      ESRI_US_FLOOD_SERVICE,
      bbox,
      { outFields: "*", resultRecordCount: 1000, maxPages: 2 },
    ...
    for (let i = 0; i < settled.length; i += 1) {
      ...
      if (result.status === "fulfilled") {
        const features = uniqueFeatures(result.value || []);
        if (features.length) {
          count += await addFloodGeoJson(features, labels[i], generation, { alpha: 0.28, zIndex: 32 });
          sources.push(labels[i]);
        }
      } else {
        warnings.push(`${labels[i]}: ${result.reason?.message || result.reason}`);
      }
    }
  ...
  async function loadFlood(location, generation = hazardGeneration) {
    if (!els.floodRisk.checked) {
      return { count: 0, source: "off", warnings: [], sourceDetails: [] };
    }

    const core = await loadCoreFloodPolygons(location, generation);
    if (generation !== hazardGeneration) {
      return { count: 0, source: "stale", warnings: [], sourceDetails: [] };
    }

    const sourceDetails = core.sources || [];
    return {
      count: core.count || 0,
  ...
  function configuredPlaySites() {
    const wanted = PLAY_DATACENTER_NAMES
    ...
    for (const wantedName of wanted) {
      const site = sites.find(candidate => {
        const officialName = String(candidate.officialName || "").trim().toLowerCase();
        const stateLabel = String(candidate.stateLabel || "").trim().toLowerCase();
        return officialName === wantedName || stateLabel === wantedName;
      });
      if (site && !matched.includes(site)) matched.push(site);
    }

    const missing = wanted.filter(name => !matched.some(site =>
      String(site.officialName || "").trim().toLowerCase() === name ||
      String(site.stateLabel || "").trim().toLowerCase() === name
    ));
    if (missing.length) console.warn("PLAY locations not found:", missing);
